...
class Target:
    def __init__(self, directory, filename_regex, table_name, aliases):
        logging.info('creating target for csv reader')
    ...
    def filter(self, f):
        logging.info(f'applying filter {self.filename.regex} to filenames')
...
class CSVReader:
    def __init__(self, root, targets=None):
        logging.info(f'creating CSVReader for directory {root}')
        self.root = root
        if targets:
            logging.info(f'target directories: {";".join(t for t in targets)}')
            self.targets = list(*targets)

    def read(self):
        for t in self.targets:
            logging.info(f'reading from subdirectory {t}')
            ...
            logging.info(f'Most recent matching file: {path_to_csv}')
